...
final class Coywolf_CVM_GitHub_Updater {

	const REPO          = 'coywolf-llc/coywolf-video-manager';
	const TRANSIENT_KEY = 'coywolf_cvm_gh_release';
	const CACHE_HOURS   = 6;
	 ...
	 */
	const CRON_HOOK = 'coywolf_cvm_refresh_release';
	...
	public function maybe_show_update_error() {
		...
		printf(
			...
			esc_html__( 'Coywolf Video Manager:', 'coywolf-video-manager' ),
			esc_html(
				sprintf(
					/* translators: %s: failure reason */
					__( 'could not check GitHub for updates (%s). This is usually temporary GitHub API rate-limiting on your host; the check retries automatically. If it persists, install the latest release manually from the plugin\'s GitHub Releases page.', 'coywolf-video-manager' ),
					$err
				)
			)
		);
	...
	public function guard_pre_download( $reply, $package, $upgrader, $hook_extra = array() ) {
		...
		if ( ! is_string( $package ) || '' === $this->validate_package_url( $package ) ) {
			return new WP_Error(
				'coywolf_cvm_untrusted_package',
				__( 'Refusing to download a plugin update from an untrusted host.', 'coywolf-video-manager' )
			);
		}
		return $reply;
	}
		...
		foreach ( $plugin_meta as $i => $item ) {
			...
			if ( false !== strpos( $item, 'plugin-install.php?tab=plugin-information' )
				...
				$plugin_meta[ $i ] = sprintf(
					...
					esc_url( $repo_url ),
					esc_html__( 'View details', 'coywolf-video-manager' )
				);
			}
		}
		return $plugin_meta;
	...
	public function plugin_info( $result, $action, $args ) {
		...
		$info                = new stdClass();
		$info->name          = 'Coywolf Video Manager';
		...
		$info->sections      = array(
			'description' => 'Manage, embed, and upload your Cloudflare Stream videos from the WordPress admin.',
	...
	public function fix_source_dirname( $source, $remote_source, $upgrader, $hook_extra = array() ) {
		...
		return new WP_Error(
			'coywolf_cvm_rename_failed',
			__( 'Could not rename the downloaded update folder to match the plugin slug.', 'coywolf-video-manager' )
		);
	}
