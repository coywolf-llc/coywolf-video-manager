...
* All Videos screen — a table of every video with views, likes, tags, and how many posts and pages embed each one.
* Tags — organize your library with tags. Add them when uploading or editing a video, or in the block; set default tags in Settings that apply automatically to every upload; and bulk-add tags to selected videos. Tags show as #tag chips in the All Videos table — click one (or type #tag in the search box) to filter the list to that tag.
* Edit Video screen — rename, add tags and a description (used in schema + the sitemap), set the creator, manage allowed origins, choose the poster (a timestamp with a live preview, or a Media Library image at the recommended size), copy the video ID, and add or AI-generate captions.
...
= 1.0.63 =
* Transparent player background by default + a Player background color setting (#64).
...
= 1.0.58 =
* ci: manual WordPress.org deploy workflow (no auto-deploy on release) (#59).
...
= 1.0.55 =
* Add an in-editor Upload video button to the block (#56).

= 1.0.54 =
* Gate anonymous play/like inserts to embedded UIDs (#55).
...
= 1.0.50 =
* Fix WordPress.org file-type compliance (extension-less + dev files) (#51).

= 1.0.49 =
* Gate Cloudflare call off public views, optimize sitemap, fix color-picker and captions accessibility (#50).

= 1.0.48 =
* Use the stored poster with an explicit time for admin, picker, and sitemap thumbnails (#49).
